 ...
 * Only call this at runtime (client-side or SSR), NOT during static build.
 */
const generateStudents = async () => {
  try {
    // Make sure API_BASE is defined
    if (!API_BASE) {
      console.warn("API_BASE is undefined. Returning empty array.");
      return [];
    }

    // Use Promise.all to fetch all required endpoints
    const [studentsRes, biographicsRes, setasRes, companiesRes] = await Promise.all([
    ...
    return students.map(student => {
      ...
      return {
        ...
        learnerships: student.programme ? `${student.programme} Learnership` : null,
        stipendAmount: null,
        lastPayment: null,
        nextPayment: null,
      };
    ...
    console.error("Student API integration failed:", error.message || error);
    return [];
  }
};
