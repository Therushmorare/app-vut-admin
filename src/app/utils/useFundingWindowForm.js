...
export default function useFundingWindowForm(initialWindow, agreementId) {
  ...
  /* ---------- API SUBMIT ---------- */
  // Accept optional callback to notify parent of success/failure
  const handleSubmit = async (callback) => {
    const validationErrors = validate();
    if (Object.keys(validationErrors).length) {
      setErrors(validationErrors);
      if (callback) callback({ success: false, errors: validationErrors });
      return;
    }
    ...
    if (!adminId) {
      ...
      if (callback) callback({ success: false, error: "Session expired" });
      return;
    }
    ...
    try {
      ...
      for (const prog of formData.programmes) {
        ...
        const progRes = await fetch(
          ...
          { method: "POST", body: progPayload, credentials: "include" }
        );
