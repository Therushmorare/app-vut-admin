...
const TABLE_API_URL = "https://seta-management-api-fvzc9.ondigitalocean.app/api/administrators/allAdmins";
const ADD_ADMIN_API_URL = "https://seta-management-api-fvzc9.ondigitalocean.app/api/administrators/add";
...
export default function AdminManagement() {
  ...
  const [apiError, setApiError] = useState(""); // <-- now inside component
  ...
  const handleAddAdmin = async (e) => {
    ...
    try {
      const response = await fetch(ADD_ADMIN_API_URL, {
        method: "POST",
  ...
  if (loading) return <div className="py-10 text-center text-gray-500">Loading admins…</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* FILTERS */}
        <div className="rounded-lg p-4 mb-4" style={{ backgroundColor: COLORS.bgWhite }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#201c52] bg-opacity-5 border-b border-gray-200">
                <tr>
                  <th onClick={() => handleSort("name")} className="text-left p-4 font-semibold cursor-pointer">
                    <div className="flex items-center space-x-1">
                      <span>Name <SortIcon field="name" /></span>
                    </div>
                  </th>
                  <th className="text-left p-4 font-semibold">Contact</th>
                  <th onClick={() => handleSort("role")} className="text-left p-4 font-semibold cursor-pointer">
                    <div className="flex items-center space-x-1">
                      <span>Role <SortIcon field="role" /></span>
                    </div>
                  </th>
                  <th onClick={() => handleSort("status")} className="text-left p-4 font-semibold cursor-pointer">
                    <div className="flex items-center space-x-1">
                      Status <SortIcon field="status" />
                    </div>
                  </th>
                  <th onClick={() => handleSort("joined")} className="text-left p-4 font-semibold cursor-pointer">
                    <div className="flex items-center space-x-1">
                ...
                {paginatedAdmins.map((admin) => (
                  <tr key={admin.admin_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm space-y-1">
                      <div className="flex items-center gap-2"><Mail className="w-4 h-4 text-gray-400" />{admin.email}</div>
                      <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-gray-400" />{admin.phone}</div>
                    </td>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-xl">
              ...
              {apiError && <p className="text-red-500 text-sm mt-1 px-6">{apiError}</p>}
